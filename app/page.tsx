...
import { BriefComparison } from '@/components/BriefComparison';
    ...
    <div style={{ minHeight: '100vh', display: 'flex', flexDirection: 'column' }}>
      ...
      >
        ...
        <span style={{ display: 'inline-flex', alignItems: 'center', gap: 7, fontSize: 12, color: 'var(--text-secondary)' }}>
          ...
          Powered by{' '}
          <a href="https://exa.ai" target="_blank" rel="noopener noreferrer" style={{ color: 'var(--accent)', fontWeight: 600 }}>
            Exa
          </a>
        </span>
      </header>
      ...
      {/* ── Main ───────────────────────────────────────────────────────── */}
      <main
        style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', padding: '48px 24px 80px' }}
      >
        {/* Primary column (search + brief) */}
        <div style={{ width: '100%', maxWidth: showBrief ? 760 : 680, transition: 'max-width 0.3s ease' }}>
          ...
          >
            ...
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 12 }}>
              ...
              <button type="submit" className="btn-primary" disabled={isLoading || !companyName.trim()}>
          ...
          {status === 'error' && (
            ...
            >
              <div>
                ...
                <div style={{ fontSize: 13, color: 'var(--text-secondary)', marginTop: 2 }}>{errorMessage}</div>
        ...
        {/* Comparison column (wider) */}
        {compareOpen && sections && (
          <div style={{ width: '100%', maxWidth: 1040, marginTop: 28 }}>
            {comparison ? (
              <>
                {/* ── Act 2: scorecard + side-by-side briefs ──────────── */}
                <div style={{ marginBottom: 14 }}>
                  <h3 style={{ margin: '0 0 2px', fontSize: 18, fontWeight: 600, color: 'var(--text-primary)' }}>
                    Exa vs. Tavily, on the same brief
                  </h3>
                  <p style={{ margin: 0, fontSize: 13, color: 'var(--text-secondary)' }}>
                    Both engines ran the identical six-section research on {comparison.company}. Tavily synthesizes
                    and structures too — so the question is quality. Here&apos;s the evidence.
                  </p>
                </div>
                ...
                {/* ── Act 3: structured comparables (reveal) ──────────── */}
                {!comparablesOpen ? (
                  <div style={{ marginTop: 28, display: 'flex', justifyContent: 'center' }}>
                    <button type="button" className="btn-primary" onClick={() => setComparablesOpen(true)}>
                      Build {comparison.company}&apos;s comp set with Exa →
                    </button>
                  </div>
                ) : (
              ...
              /* ── Live (non-seeded) fallback: Exa brief vs Tavily Search ── */
              <>
                <div style={{ marginBottom: 12 }}>
                  <h3 style={{ margin: '0 0 2px', fontSize: 15, fontWeight: 600, color: 'var(--text-primary)' }}>
                    Exa Deep Research vs. Tavily Search
                  </h3>
                  <p style={{ margin: 0, fontSize: 13, color: 'var(--text-secondary)' }}>
                    A live look at Tavily&apos;s fast Search layer (raw retrieval). Tavily also offers a Research
                    endpoint that synthesizes — see the Stripe example for the full head-to-head.
                  </p>
                </div>
                <ComparePanel
                  sections={sections}
